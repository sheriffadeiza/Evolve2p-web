...
const TransLim: React.FC = () => {
  ...
  // Helper to calculate percentage (for progress bar) – always 0% because no usage data
  const calculatePercentage = () => 0;
            ...
            <div className="mt-4 md:mt-[20px] p-2 md:p-[10px_20px]">
              ...
              {activeTab === "withdrawal" ? (
                <div className="mt-4 md:mt-[10px]">
                  <h3 className="text-lg sm:text-xl md:text-[20px] font-[700] text-[#FFFFFF] mt-6 md:mt-[40px]">Withdrawal Limit</h3>
                  <p className="text-sm md:text-[16px] font-[400] text-[#C7C7C7] mb-6">
                    Daily limit for sending money from your balances
                  </p>

                  {/* Daily Limit Card */}
                  <div className="w-full max-w-[641px] h-auto bg-[#222222] rounded-lg p-4 md:p-5 mb-4">
                    <div className="flex justify-between mb-2">
                      <p className="text-sm font-medium text-[#FFFFFF]">
                        Daily Limit: <span className="text-[#4DF2BE] font-bold">{formatCurrency(settingsData?.withdrawalLimit)}</span>
                      </p>
                      {/* Request Increase button removed */}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between mt-3 text-sm text-[#B5B5B5] gap-2">
                      <span>{formatCurrency(settingsData?.withdrawalLimit)} remaining</span>
                      <span>Usage tracking not available</span>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="mt-4 md:mt-[10px]">
                  <h3 className="text-lg sm:text-xl md:text-[20px] font-[700] text-[#FFFFFF] mt-6 md:mt-[40px]">Deposit Limit</h3>
                  <p className="text-sm md:text-[16px] font-[400] text-[#C7C7C7] mb-6">
                    Daily limit for making payments into your balances
                  </p>
                  ...
                  <div className="w-full max-w-[641px] h-auto bg-[#222222] rounded-lg p-4 md:p-5 mb-4">
                    <div className="flex justify-between mb-2">
                      <p className="text-sm font-medium text-[#FFFFFF]">
                        Daily Limit: <span className="text-[#4DF2BE] font-bold">{formatCurrency(settingsData?.depositLimit)}</span>
                      </p>
                      {/* Request Increase button removed */}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between mt-3 text-sm text-[#B5B5B5] gap-2">
                      <span>{formatCurrency(settingsData?.depositLimit)} remaining</span>
                      <span>Usage tracking not available</span>
                    </div>
                  </div>
                </div>
