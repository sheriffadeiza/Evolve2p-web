...
const Lverifybd: React.FC = () => {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (!otpRes.ok) {
        setError(otpData.detail || otpData.message || 'Failed to resend code. Please try again.');
      }
    } catch {
      setError('Network error. Please try again.');
    } finally {
  ...
  return (
    <div className="max-w-md mx-auto ml-[-80px] text-center mt-10 px-4 text-white">
      ...
      <form onSubmit={handleVerify}>
        ...
        {error && <div className="text-[#F5918A] text-[14px] font-[500] mb-2">{error}</div>}
      ...
      {showModal && (
        <div className="fixed inset-0 flex ml-[20%] w-[80%] mt-[-20%] items-center justify-center bg-black bg-opacity-60 z-50">
          <div className="bg-[#1F1F1F] rounded-lg p-8 shadow-lg text-center flex flex-col items-center justify-center">
            <h2 className="text-[#1ECB84] text-[20px] font-[700] mb-2">Email Verified!</h2>
